...
function Channel(chId, sinc) {
    this._chId = chId;
    this._chIdFull = sinc._id + ':' + chId;
    this._sinc = sinc;
    this._nodes = {};
    sinclog.info('New channel: chId=' + this._chId + ' chIdFull=' + this._chIdFull);
}
...
Channel.prototype.send = function(hdr, msg, options) {
    ...
    self._sinc.cli.publish(self._chIdFull, buf.toString('binary'));
};
